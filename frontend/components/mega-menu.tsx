    ...
    return (
      ...
      >
        {items.map((navItem) => (
          ...
          >
            <button
              className="relative flex cursor-pointer items-center justify-center gap-1 py-1.5 px-4 text-sm rounded-2xl  text-white transition-colors duration-300 hover:text-white group"
            ...
            >
              ...
              {(isHover === navItem.id || openMenu === navItem.label) && (
                <motion.div
                  layoutId="hover-bg"
                  className="absolute inset-0 size-full bg-black/10"
                  style={{
                    borderRadius: 99,
                  }}
                />
            ...
            <AnimatePresence>
              {openMenu === navItem.label && navItem.subMenus && (
                <div className="absolute left-0 top-full w-auto pt-2 z-10">
                  <motion.div
                    className="w-max border border-white/10 bg-blue-500 p-4"
                    style={{
                      borderRadius: 16,
                    }}
                    layoutId="menu"
